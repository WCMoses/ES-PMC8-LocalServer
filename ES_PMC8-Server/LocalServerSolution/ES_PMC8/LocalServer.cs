//
// ASCOM.ES_PMC8 Local COM Server
//
...
namespace ASCOM.ES_PMC8
{
    public static class Server
    {
        ...
        private static string s_appId = "{4ab4b6f8-15f6-4a5d-930c-fc03168e28bf}";	// Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "ES_PMC8", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The ES_PMC8 was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "ES_PMC8", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "ES_PMC8", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "ES_PMC8", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
        ...
        static void Main(string[] args)
        {
            ...
            MessageBox.Show("Attach Debugger 3 - From server", "Message" + Environment.NewLine + dir);

            bool open = IsProcessOpen("ASCOM.PMC8_Server.exe");
            //if (!open)
            //{
            //    string dir2 = Directory.GetCurrentDirectory() + @"\";
            //    Process.Start(dir2 + "ASCOM.PMC8_Server.exe");
            //}
